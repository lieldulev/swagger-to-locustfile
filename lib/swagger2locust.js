...
module.exports = {
  ...
  convertSpec : function(swgObj, options) {
    ...
    if (swgObj['x-locust-import']) { actualOptions['x-locust-import'] = swgObj['x-locust-import']}
    locustfile += this.locustfileHeader(actualOptions);
  ...
  HTTPLocustGetFromPath : function(pathName, pathObj, basePath) {
    ...
    var resolvedPath = this.resolvedPathWithDefaults(pathName, pathObj);
    task += '        self.client.get("'+basePath+resolvedPath.path+'"'+(resolvedPath.paramsValues.length > 0 ? ('.format('+resolvedPath.paramsValues.join(',')+')') : '')+')'+"\n\n";
  ...
  resolvedPathWithDefaults : function(pathName, pathObj) {
    ...
    if (pathName.indexOf("{") != -1) {
      ...
      for (currParam in pathParamsNames) {
        ...
        currPlaceholder += 1;
        requiredParamValues.push(this.getParamDefault(pathParamsNames[currParam], pathObj['parameters']));
      }
    }
    ...
    for (currPathParam in pathObj['parameters']){
      if (pathObj['parameters'][currPathParam]['in'] == 'query' && pathObj['parameters'][currPathParam]['required']) {
        requiredQSParams.push(pathObj['parameters'][currPathParam]['name']+'={'+currPlaceholder+'}');
        currPlaceholder += 1;
        if (pathObj['parameters'][currPathParam]['x-locust-value']) {
          requiredParamValues.push(pathObj['parameters'][currPathParam]['x-locust-value']);
        } else {
          requiredParamValues.push('"'+pathObj['parameters'][currPathParam]['default']+'"');
        }
         // +pathObj['parameters'][currPathParam]['default']);
      }
    }
  ...
  getParamDefault : function(paramName, pathParameters)  {
    for (currPathParam in pathParameters) {
      if (pathParameters[currPathParam]['name'] == paramName) {
        if (pathParameters[currPathParam]['x-locust-value']) {
          return pathParameters[currPathParam]['x-locust-value'];
        } else {
